...
// (121:4) {#each $loadedFiles as file}
function create_each_block(ctx) {
	...
	const block = {
		c: function create() {
			...
			t3 = space();
			add_location(button, file, 123, 8, 3518);
			add_location(li, file, 121, 6, 3489);
		},
	...
	dispatch_dev("SvelteRegisterBlock", {
		...
		type: "each",
		source: "(121:4) {#each $loadedFiles as file}",
		ctx
	});
...
function create_fragment(ctx) {
	...
	const block = {
		c: function create() {
			...
			add_location(h1, file, 111, 0, 3254);
			add_location(h20, file, 114, 2, 3288);
			attr_dev(input, "type", "file");
			input.multiple = true;
			add_location(input, file, 115, 2, 3309);
			add_location(p0, file, 116, 2, 3398);
			add_location(h3, file, 118, 2, 3424);
			add_location(ul, file, 119, 2, 3445);
			add_location(div0, file, 113, 0, 3280);
			add_location(h21, file, 130, 2, 3621);
			attr_dev(textarea, "placeholder", "궁금한 점을 물어보세요");
			add_location(textarea, file, 131, 2, 3635);
			add_location(button, file, 132, 2, 3714);
			add_location(p1, file, 133, 2, 3762);
			add_location(div1, file, 129, 0, 3613);
		},
...
function instance($$self, $$props, $$invalidate) {
	...
	// API_KEY를 환경 변수에서 직접 불러오는 대신 수동으로 설정
	const API_KEY = import.meta.env.VITE_OPENAI_API_KEY; // Vite 사용 시 `import.meta.env`를 통해 환경 변수 접근 가능
	...
	onMount(async () => {
		try {
			pdfjsLib = await import('./pdf-c313c183.js');
			pdfjsLib.GlobalWorkerOptions.workerSrc = `//cdnjs.cloudflare.com/ajax/libs/pdf.js/${pdfjsLib.version}/pdf.worker.min.js`;
		} catch(error) {
			$$invalidate(0, statusMessage = `PDF.js 라이브러리를 로드하는 중 오류 발생: ${error.message}`);
		}
	});
